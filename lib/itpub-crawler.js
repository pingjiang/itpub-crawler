...
ITPubClient.prototype.pagesOfThread = function(url, done) {
  this.doRequest('get', url, function(err, res, data) {
    if (err) {
      return done(err);
    }
    ...
    var $ = cheerio.load(data);
    var pg = $('div#pgt div.pg'), pagedLink = url;
    if (pg && pg.length === 1) {
      var anchors = $('a.last', pg);
      if (anchors && anchors.length > 0) {
        var lastAnchor = ITPUB_URL_BASE + anchors.first().attr('href');
        pagedLink = lastAnchor.replace(/thread\-([^\-]+)\-([^\-]+)/, 'thread-$1-[1-$2]');
      } else {
        var lessAnchors = $('a', pg);
        var total = lessAnchors ? lessAnchors.length : 1;
        if (total > 1) {
          pagedLink = url.replace(/thread\-([^\-]+)\-([^\-]+)/, 'thread-$1-[1-' + total + ']');
        }
      }
    }
    done(null, pagedLink);
...
ITPubClient.prototype.searchThreadAttachments = function(threadUrl, link, done) {
  var self = this;
  self.pagesOfThread(link.href, function(err, pagedLink) {
    var urls = self.expandUrl(pagedLink).map(function(paged) {
      return { href: paged, text: link.text };
    });
    
    urls.forEach(function(alink) {
      self.doRequestAndParseLinks(alink.href, 'div#postlist a[href^="attachment.php?"]', function(err, res, links) {
        if (err) {
          return done(err);
        }
        done(null, threadUrl, alink, links);
      });
    });
  });
};
